...
if __name__ == '__main__':
    ...
    mr_mask_path = '/root/data1/spine/public/data/fine_weak_true'
    mrDir = os.path.join(mr_mask_path, 'in/nii/original_mr')
    maskDir = os.path.join(mr_mask_path, 'in/nii/mask')
    ...
    for fold_ind in range(1, 2):
        ...
        for i in train_ind:
            ...
            for ii in range(mr.shape[0]):
                # filename = 'case_' + str(i) + '_' + str(ii+1) + '_12345'
                filename = 'case_' + str(i) + '_' + str(ii+1)
                # filename = np.array(filename.encode('ascii'))
            ...
            if flag:
                ...
                # g_train.create_dataset('filename', data=filename_list, maxshape=train_slice_num, chunks=1)
                g_train.create_dataset('filename', data=filename_list, maxshape=(train_slice_num,), dtype=h5py.string_dtype(encoding='utf-8'))
            else:
                ...
                g_train['filename'].resize(g_train['filename'].shape[0] + filename_list.shape[0], axis=0)
                g_train['filename'][-filename_list.shape[0]:] = filename_list

                print('====================================')
                filename_enc = f['train']['filename']
                print(filename_enc[file_num+10].decode('utf-8'))
                # print(filename_enc[file_num + 10])
            ...
            #         filename = 'case_' + str(i) + '_' + str(ii + 1)
            #         filename = np.array(filename.encode())
            #         filename_list.append(filename)
            #     filename_list = np.array(filename_list)
            #     # unary = np.load(os.path.join(unaryDir, 'Case' + str(i) + '_logit.npz'))['logit']  # [1, 20, d, 128, 256]  # --------------------------------------------------------------------------------------------
            #
            ...
            #                                chunks=(1, mask.shape[1], mask.shape[2]))
            #
            #         g_val.create_dataset('filename', data=filename_list, maxshape=val_slice_num, chunks=1)
            #     else:
            ...
            #
            #         g_val['filename'].resize(g_val['filename'].shape[0] + filename_list.shape[0], axis=0)
            #         g_val['filename'][-filename_list.shape[0]:] = filename_list
            ...
            #                              chunks=(1, mask.shape[1], mask.shape[2]))
            #
            #         g_test.create_dataset('filename', data=filename_list, maxshape=test_slice_num, chunks=1)
            #     else:
            ...
            #
            #         g_test['filename'].resize(g_test['filename'].shape[0] + filename_list.shape[0], axis=0)
            #         g_test['filename'][-filename_list.shape[0]:] = filename_list
