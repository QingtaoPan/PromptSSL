...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=1, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.conv1 = nn.Conv2d(in_channels=in_chans, out_channels=768, kernel_size=patch_size, stride=patch_size, bias=False)
    ...
    def forward(self, x, register_blk=-1):
        ...
        x = self.conv1(x)  # [b, 768, 16, 32]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # [b, 768, 512]
        x = x.permute(0, 2, 1)  # [b, 512, 768]
