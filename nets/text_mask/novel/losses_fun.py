...
def masked_pool(spatial_image_emb, mask, eps=1e-6):  # spatial_image_emb:[b, 512 64, 128], mask:[b, b, 64, 128]--float
    ...
    weight = mask / (mask_sum + eps)  # [b, b, 64, 128]
...
# 特征级损失
def tclf_loss_fun(s1_image_emb, mask, s1_text_emb):  # s1_image_emb[b, 512 64, 128], mask:[b, b, 64, 128]--float, s1_text_emb[b, 512]
...
# 图像级别损失-输入数据类型为half()
def tcli_loss_fun(mask_pos, image, text_emb, mask_encoder, vision_proj):  # [b, 1, 56, 56]-->[b, 1, 64, 128]-hard_mask, [b, 1, 256, 512]-image, [b, 512]-text_feat
    ...
    masked_img_emb = mask_encoder(mask_image)  # 输入:pos_mask*image[b, 1, 256, 512],  输出:[b, 513, 768]
