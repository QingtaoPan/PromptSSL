...
class FeatureEncoder(nn.Module):
    ...
    def forward(self, *args, ret_feats=False, **kwargs):
        self.clear_features()
        x = self._encode(*args, **kwargs)  # [b, 768, 14, 14]
        if ret_feats:  # True
            if self.safe:  # True
                features = [t.clone() for t in self._features]  # 获取12层中resnet在指定层的特征
                print(x, x.shape)
...
class LayerNorm2d(nn.LayerNorm):
    def __init__(self, num_channels, eps=1e-5, affine=True):  # num_channels:768
...
class ConvBlock(nn.Module):
    def __init__(
        ...
        self.norm = get_norm_fn(norm, C_in)  # 'ln', 768
        self.activ = activ()
        if dropout > 0.0:
...
class ResConv(nn.Module):
    ...
    def __init__(
        ...
        self.C_in = C_in  # 768
        self.C_out = C_out  # 768
        self.upsample = upsample  # True
        self.double = double  # True
        self.conv = ConvBlock(
