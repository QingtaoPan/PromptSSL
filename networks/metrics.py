...
class DiceCoefficient:
    ...
    def __init__(self, epsilon=1e-5, ignore_index=None, skip_channels=None, **kwargs):
        ...
        self.skip_channels = skip_channels  # None

    def __call__(self, input, target):  # input[1, 20, 256, 512], target[1, 20, 256, 512]
        """
        ...
        n_classes = input.size()[1]  # 20
        if input.dim() == 5 and target.dim() == 4:
            target = expand_as_one_hot(target, C=n_classes, ignore_index=self.ignore_index)
        elif input.dim() == 4 and target.dim() == 3:  # 执行该句--------------------------------------------
            target = expand_as_one_hot(target, C=n_classes, ignore_index=self.ignore_index)
...
def get_evaluation_metric_disc(config):
    """
    ...
    def _metric_class(class_name):  # DiceCoefficient
        m = importlib.import_module('networks.metrics')
        clazz = getattr(m, class_name)
        return clazz

    assert 'eval_metric_disc' in config, 'Could not find evaluation metric configuration'
    metric_config = config['eval_metric_disc']
    metric_class = _metric_class(metric_config['name_disc'])  # DiceCoefficient
    return metric_class(**metric_config)

def get_evaluation_metric_miou(config):
    """
    Returns the evaluation metric function based on provided configuration
    :param config: (dict) a top level configuration object containing the 'eval_metric' key
    :return: an instance of the evaluation metric
    ...
    assert 'eval_metric_miou' in config, 'Could not find evaluation metric configuration'
    metric_config = config['eval_metric_miou']
    metric_class = _metric_class(metric_config['name_miou'])  # DiceCoefficient
    return metric_class(**metric_config)
