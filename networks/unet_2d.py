...
class ResidualUNet2D(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid, f_maps=32, conv_layer_order='cge', num_groups=8,  # in_channels=1, out_channels=20, final_sigmoid=Fasle, f_maps=32, conv_layer_order='cbr', num_groups=8
                 **kwargs):
        ...
        if isinstance(f_maps, int):
            ...
            f_maps = create_feature_maps(f_maps, number_of_fmaps=5)  # [32, 64, 128. 256, 512]

        # create encoder path consisting of Encoder modules. The length of the encoder is equal to `len(f_maps)`
        # uses ExtResNetBlock as a basic_mod ule for the Encoder
        encoders = []
        for i, out_feature_num in enumerate(f_maps):
            if i == 0:
    ...
    def forward(self, img, unary=None, **kwargs):
        ...
        # x[16, 1, 256,, 512]
        ########--下采样---########-->feature0:[16, 32, 256,, 512], feature1:[16, 64, 128,, 256], feature2:[16, 128, 64, 128], feature3:[16, 256, 32, 64], feature4:[16, 512, 16, 32]
        for en_num, encoder in enumerate(self.encoders):
            x = encoder(x)
            # reverse the encoder outputs to be aligned with the decoder
            encoders_features.insert(0, x)
        ########--下采样---########-->输出 feature4:[16, 512, 16, 32]
        ...
        for decoder, encoder_features in zip(self.decoders, encoders_features):
            ...
            # of the previous decoder
            x = decoder(encoder_features, x)
        ########--上采样---########-->输出 [16, 32, 256, 512]

        final_conv = self.final_conv(x)  # [16, 32, 256, 512]-->[16, 20, 256, 512]
        ...
        if not self.training:
            ...
            return final_conv, None  # [16, 20, 256, 512]
