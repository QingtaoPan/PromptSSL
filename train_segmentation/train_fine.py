...
from datasets.fine_h5_outbatch import load_data
import os
...
from networks.metrics import get_evaluation_metric_disc, get_evaluation_metric_miou
import importlib
from networks.trainer_2d_outbatch import Trainer
...
def get_parser():
    ...
    parser.add_argument("--augment", dest='augment', action='store_true',
                        help="whether use augmentation")  # 数据增强
    ...
    parser.set_defaults(use_unary=False)
    ...
    parser.add_argument("--iters", type=int, default=1000000000,
    ...
    parser.add_argument('--eval_metric_disc', type=str, default='DiceCoefficient',
                        help="The eval_metric name, MeanIoU or DiceCoefficient")

    parser.add_argument('--eval_metric_miou', type=str, default='MeanIoU',
                        help="The eval_metric name, MeanIoU or DiceCoefficient")
...
def _create_trainer(config, model, model_2, optimizer, lr_scheduler, loss_criterion, eval_criterion_disc, eval_criterion_miou, train_loader_weak, train_loader_sup, val_loader, logger):
    ...
    if resume is not None:
        ...
        return Trainer.from_checkpoint(resume, model,
                                             ...
                                             eval_criterion_disc, eval_criterion_miou, train_loader_weak, train_loader_sup, val_loader,
                                             logger=logger)
        ...
        return Trainer.from_pretrained(pre_trained, model, optimizer, lr_scheduler, loss_criterion,
                                             eval_criterion_disc, eval_criterion_miou, device=config['device'],
                                             train_loader_weak=train_loader_weak, train_loader_sup=train_loader_sup, val_loader=val_loader,
        ...
        return Trainer(model, model_2, optimizer, lr_scheduler, loss_criterion, eval_criterion_disc, eval_criterion_miou, config['device'], train_loader_weak, train_loader_sup, val_loader, trainer_config['checkpoint_dir'],
                       max_num_epochs=trainer_config['epochs'], batch_size=trainer_config['batch_size'], max_num_iterations=trainer_config['iters'], validate_after_iters=trainer_config['validate_after_iters'],
                       log_after_iters=trainer_config['log_after_iters'], eval_score_higher_is_better=trainer_config['eval_score_higher_is_better'], logger=logger)
...
def main():
    ...
    if args.coarse_identifier == 'DeepLabv3_plus_gcn_skipconnection_3d_gcn_mode_2_ds_weight_0.3_loss_' \
        ...
        data_path = os.path.join(data_dir, 'in/h5py/fold' + str(fold_ind) + '_data.h5')  # /public/pangshumao/data/five-fold/fine/in/h5py/fold_1_data.h5
    else:
    ...
    conf['loss'] = {}
    conf['loss']['name'] = args.loss  # CrossEntropyLoss

    conf['eval_metric_disc'] = {}
    conf['eval_metric_disc']['name_disc'] = args.eval_metric_disc  # DiceCoefficient
    conf['eval_metric_disc']['skip_channels'] = args.skip_channels  # None

    conf['eval_metric_miou'] = {}
    conf['eval_metric_miou']['name_miou'] = args.eval_metric_miou  # MeanIoU
    conf['eval_metric_miou']['skip_channels'] = args.skip_channels  # None
    ...
    if args.model == 'UNet2D':
        ...
        model = ResidualUNet2D(in_channels=in_channels, out_channels=20, final_sigmoid=False, f_maps=32, conv_layer_order='cbr', num_groups=8)
        model_2 = ResidualUNet2D(in_channels=in_channels, out_channels=20, final_sigmoid=False, f_maps=32, conv_layer_order='cbr', num_groups=8)
    ...
    model_2 = model_2.to(conf['device'])

    model_student = choose_model(model_type='student', model_=model)
    model_teacher = choose_model(model_type='teacher', model_=model_2)
    # Log the number of learnable parameters
    logger.info(f'Number of learnable params {get_number_of_learnable_parameters(model)}')

    # Create loss criterion
    loss_criterion = get_loss_criterion(conf)  # 损失指标-------------------------------------------------------------------------------------------------------------------------------------------------
    # Create evaluation metric
    eval_criterion_disc = get_evaluation_metric_disc(conf)  # 性能指标-------------------------------------------------------------------------------------------------------------------------------------------------
    eval_criterion_miou = get_evaluation_metric_miou(conf)


    if args.augment:
        print('---------------------- 执行了数据增强 ---------------------------------')
        train_data_loader_weak, train_data_loader_sup, val_data_loader, ff = load_data(data_path, batch_size=batch_size,  # data_path-->/public/pangshumao/data/five-fold/fine/in/h5py/fold_1_data.h5
                                                                            transformer_config=conf['transformer'],
                                                                            return_weight=return_weight)  # return_weight-->False
    else:
        train_data_loader_weak, train_data_loader_sup, val_data_loader, ff = load_data(data_path, batch_size=batch_size,
                                                                            return_weight=return_weight)
    conf['trainer']['validate_after_iters'] = len(train_data_loader_weak)
    conf['trainer']['log_after_iters'] = len(train_data_loader_weak)
    ...
    trainer = _create_trainer(conf, model=model_student, model_2=model_teacher, optimizer=optimizer, lr_scheduler=lr_scheduler,
                              loss_criterion=loss_criterion, eval_criterion_disc=eval_criterion_disc, eval_criterion_miou=eval_criterion_miou,
                              train_loader_weak=train_data_loader_weak, train_loader_sup=train_data_loader_sup,
