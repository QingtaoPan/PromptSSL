...
def get_parser():
    ...
    parser.add_argument("--fold_ind", type=int, default=1,  # 改变折数时需要改变-----------------------------------------------------------------------------
                        help="1 to 5")

    parser.add_argument("--method_fold", type=str, default='baseline_fold_1',  # 改变折数时需要改变-----------------------------------------------------------------------------
                        help="the data dir")

    parser.add_argument("--data_dir", type=str, default='/root/data1/spine/public/data',
                        help="the data dir")
...
if __name__ == '__main__':
    ...
    train_ind = []
    val_ind = []
    for p in range(1, total_samples + 1):
        train_ind.append(p)
    for q in range((each_fold) * (fold_ind- 1) + 1, (each_fold) * fold_ind + 1):
        val_ind.append(q)
    for item in val_ind:
        train_ind.remove(item)
    half_samples = int(len(train_ind) / 2)
    train_ind_weak, train_ind_sup = train_ind[:half_samples], train_ind[half_samples:]

    num_classes = 20
    data_dir = args.data_dir
    coarse_dir = os.path.join(data_dir, 'coarse')
    fine_dir = os.path.join(data_dir, 'fine_weak_true')  # 修改-------------------------------------------------------
    ...
    mrDir = os.path.join(data_dir, 'fine_weak_true', 'in/nii/original_mr')
    maskDir = os.path.join(data_dir, 'fine_weak_true', 'in/nii/mask')
    # pre_seg_dir = os.path.join(coarse_dir, 'out', 'fold' + str(fold_ind), args.coarse_identifier)

    if args.coarse_identifier == 'DeepLabv3_plus_gcn_skipconnection_3d_gcn_mode_2_ds_weight_0.3_loss_' \
                                 'CrossEntropyLoss_Adam_lr_0.001_pretrained':
        fold_dir = os.path.join(fine_dir, 'out_'+args.method_fold, 'fold' + str(fold_ind))
    else:
    ...
    if args.model == 'non_local_crf':
        ...
        identifier = args.model + '_' + args.optimizer + '_lr_' + str(args.learning_rate) + '_weight_decay_' + str(  # 执行该句----------------------------------------
            args.weight_decay)
        if args.use_unary is False:  # 执行该句------------------------------
            identifier = identifier + '_noUnary'
            in_channels = 1
        else:
            in_channels = 21
    if args.augment:  # 执行该句----------------------------------------
    ...
    if args.coarse_identifier == 'DeepLabv3_plus_gcn_skipconnection_3d_gcn_mode_2_ds_weight_0.3_loss_' \
                                 'CrossEntropyLoss_Adam_lr_0.001_pretrained':   # 执行该句-----------------------------------
        model_dir = os.path.join(data_dir, 'fine_weak_true', 'model_'+args.method_fold, 'fold' + str(fold_ind), identifier)   # 修改--执行该句-----------------------------------
    else:
        model_dir = os.path.join(fine_dir, 'model', 'fold' + str(fold_ind) + '_' + args.coarse_identifier, identifier)
    model_dir = model_dir + '_augment'
    ...
    try:
        train_data_loader_weak, train_data_loader_sup, val_data_loader, f = load_data(data_path, batch_size=1, shuffle=False)

        model.eval()

        if args.dimension == 2:
            crf_slices = []
            for i, t in enumerate(val_data_loader):
                mr, _, _, _ = t
                mr = mr.to(conf['device'])
                output, _, _ = model(unary=None, img=mr)
            ...
            for i in val_ind:
                ...
                crf_miou = utils.miou_all_class(seg, h_mask, class_num=20)
                print('-----------------:', seg.shape, h_mask.shape)

                # unary_dices.append(unary_dice)
                crf_dices.append(crf_dice)
                crf_mious.append(crf_miou)
                # print('case %d, unary Dice = %.4f, crf Dice = %.4f' % (i, unary_dice, crf_dice))
                print('case %d, crf Dice = %.4f, crf miou = %.4f' % (i, crf_dice, crf_miou))
            ...
            print('mean crf Dice = %.4f, mean crf miou = %.4f' % (np.mean(crf_dices), np.mean(crf_mious)))  # 修改的print
            ...
            np.savez(os.path.join(out_dir, 'eval_scores.npz'), crf_dices=crf_dices, mean_crf_dice=np.mean(crf_dices), crf_mious=crf_mious, mean_crf_mious=np.mean(crf_mious))
